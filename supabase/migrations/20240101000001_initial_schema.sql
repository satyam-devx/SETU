...
-- ─────────────────────────────────────────────────────────
-- DISPUTES / ESCALATIONS / NOTICEBOARD
-- These anchor-portal tables are defined in migration 006
-- (anchor_portal) using the canonical, richer schema the app relies on
-- (disputes.reporter_id/title/amount/resolved_by, noticeboard.created_by,
-- plus the dispute_parties table). The earlier duplicate definitions
-- that lived here — and in migration 007 — used a divergent older schema
-- (raised_by/author_id) and collided on a fresh apply, so they were
-- removed. Migration 006 is now the single owner. Realtime publication
-- membership for disputes/noticeboard is (re)added idempotently in 007.
-- ─────────────────────────────────────────────────────────
...
-- disputes / noticeboard join the realtime publication in migration 007
-- (idempotent), after migration 006 creates those tables.
-- ═══════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════
-- DISPUTES / ESCALATIONS / NOTICEBOARD — RLS
-- Policies live in migration 006 (anchor_portal), which owns these
-- tables with the app's canonical schema (reporter_id / created_by /
-- dispute_parties). The duplicate policies that were here referenced the
-- removed raised_by/author_id columns, so they were removed too.
-- ════════════════════════════════════════════════════════
